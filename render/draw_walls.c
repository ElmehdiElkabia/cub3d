...
void	draw_wall_slice(t_game *d, t_ray *r, int x)
{
	...
	line_height = (int)(IMAGE_HIEGHT / r->dist);
	draw_start = -line_height / 2 + IMAGE_HIEGHT / 2;
	if (draw_start < 0)
		draw_start = 0;
	draw_end = line_height / 2 + IMAGE_HIEGHT / 2;
	if (draw_end >= IMAGE_HIEGHT)
		draw_end = IMAGE_HIEGHT - 1;
	if (r->hit_side == 0)
		color = 0xAAAAAA;
	else
		color = 0x888888;
